...
node(EC2 machine) to send a task to "worker" machines. It is important
to note, that both master and worker nodes are on EC2. The user would
have to start an instance, manually, and use this machine as master.
...
user needs an AWS access key and secret key.
...
2. Give the AWSParallel param, the new subnet, and a security group as described
