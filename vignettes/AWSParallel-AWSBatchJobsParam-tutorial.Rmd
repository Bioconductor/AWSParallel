...
These AWS credentials should be stored on your HOST machine accessible at
...
The setting `[default]` which you see on the AWS credentials, determines
the profile of the AWS credentials. If you have multiple profiles,
be sure to specify the correct profile in the argument to instantiate
the AWSBatchJobsParam. 
...
need to be on the same VPC and subnet with permissible security groups.
Without this the connection established between the machines 
...
The user needs to start a HOST instance on AWS directly from his
AWS account using the AWS UI to set up the host. This host instance 
will be used as an interface to control the AWS Cluster.
...
jobs. If you launch a large job only on the "master" node, there is no increment in speed if the job is not parallelized.
